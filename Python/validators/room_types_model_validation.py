# room_types_model_validation.py 
...
def validate_room_type(room_type: str) -> None:
    ...
    validate_room_type("")  # ValueError: Nazwa typu pokoju nie może być pusta.
    """
    ...
    :example:
        validate_filters_and_sorting(
            filters=[
                ...
                {"column": "room_type_id", "operator": "IN", "value": [1, 2, 3]},
            ],
            sort_by=[("room_type", "ASC")],
            valid_columns=["room_type", "room_type_id"]
        )  # Brak błędów
    """
...
def validate_unique_room_type(db_controller, room_type: str):
    """
    Sprawdza, czy nazwa typu pokoju jest unikalna w bazie danych.

    :param db_controller: Obiekt kontrolera bazy danych.
    :param room_type: Nazwa typu pokoju do sprawdzenia.
    :raises ValueError: Jeśli nazwa typu pokoju już istnieje w bazie danych.
    :example:
    validate_unique_room_type(db_controller, "Konsylium terapeutyczne")  # Brak błędu
    """
    query = "SELECT COUNT(*) FROM room_types WHERE room_type = ?"
    cursor = db_controller.connection.execute(query, (room_type,))
    if cursor.fetchone()[0] > 0:
        raise ValueError(f"Typ pokoju o nazwie '{room_type}' już istnieje.")
...
# Metoda get_valid_columns w pliku room_types.py:
